...
export function stepZoom(currentZoom, direction, options = {}) {
  const { min = 0.25, max = 4.0, ratio = 1.15 } = options;
  const proposed = direction > 0 ? currentZoom * ratio : currentZoom / ratio;
  return Math.min(max, Math.max(min, proposed));
}
